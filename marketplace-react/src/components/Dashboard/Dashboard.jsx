...
const Dashboard = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [reqsRes, offersRes, propsRes] = await Promise.all([
      ...
      setProposals(propsRes.data.filter(p => p.emp?.empId === empId));
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
          ...
          <div className="animate-slide-up">
            ...
            <div className={styles.tabContent}>
              {activeTab === 'requirements' && (
                <>
                  {requirements.length === 0 ? (
                    <EmptyState icon={<ClipboardList size={48} />} message="No requirements yet. Post your first requirement!" />
                  ) : (
              ...
              {activeTab === 'offers' && (
                <>
                  {offers.length === 0 ? (
                    <EmptyState icon={<PackageOpen size={48} />} message="No offers yet. Post your first offer!" />
                  ) : (
                    <div className="grid-auto animate-fade-in">
                      {offers.map(offer => (
              ...
              {activeTab === 'proposals' && (
                <>
                  {proposals.length === 0 ? (
                    <EmptyState icon={<Handshake size={48} />} message="You haven't submitted any proposals yet." />
                  ) : (
                    <div className="grid-auto animate-fade-in">
                      {proposals.map(prop => (
