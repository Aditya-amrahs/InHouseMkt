...
|---|---|---|
| **Mehul Agarwal** | Full-Stack | Backend architecture, all service + repository layers, REST controllers, JaCoCo coverage, UI design system |
| **Aditya Sharma** | Backend + Frontend | Offer CRUD, Proposal CRUD and acceptance flow, React integration, GitHub repository management |
| **Srinidhi Rao** | Backend | User authentication, Employee profile, Requirement CRUD, repository layer |
...
Presentation  +--------------------------------------------------+
              ...
              |          OfferForm                               |
              |                                                  |
              |  State:  AuthContext (localStorage + sync)        |
...
**Owner:** Srinidhi Rao | **Est:** 4 hrs | **Sub-task:** Create `User` entity + `POST /api/users/register`
...
- Session state persisted in `localStorage` so authenticated tabs stay in sync
- All marketplace routes redirect to `/login` when no session exists

**Owner:** Srinidhi Rao | **Est:** 4 hrs | **Sub-task:** `POST /api/users/login` + React `AuthContext` + `ProtectedRoute`
...
**Owner:** Srinidhi Rao | **Est:** 5 hrs | **Sub-task:** `Requirement` entity + `POST /api/requirements`
...
**Owner:** Srinidhi Rao | **Est:** 4 hrs | **Sub-task:** `PUT /api/requirements` + React edit form
...
**Owner:** Srinidhi Rao | **Est:** 3 hrs | **Sub-task:** `DELETE /api/requirements/{id}` + `PATCH /employees/requirements/fulfilled`
...
**Owner:** Srinidhi Rao | **Est:** 6 hrs | **Sub-task:** `POST /api/offers`
...
**Owner:** Aditya Sharma | **Est:** 4 hrs | **Sub-task:** `PUT /api/offers`
...
**Owner:** Aditya Sharma | **Est:** 4 hrs | **Sub-task:** `DELETE /api/offers/{id}` + `PATCH /employees/offers/availability`
...
**Owner:** Aditya Sharma | **Est:** 6 hrs | **Sub-task:** `POST /api/proposals`
...
**Owner:** Aditya Sharma | **Est:** 4 hrs | **Sub-task:** `PATCH /employees/proposals/accepted`
...
**Owner:** Aditya Sharma | **Est:** 3 hrs | **Sub-task:** `DELETE /api/proposals/{id}`
...
**Owner:** Mehul Agarwal | **Est:** 8 hrs | **Sub-task:** JPA `@Repository` interfaces — User, Employee, Requirement, Offer, Proposal, Resource
...
**Owner:** Mehul Agarwal | **Est:** 10 hrs | **Sub-task:** Service implementations + Mockito unit tests for all modules
...
**Owner:** Mehul Agarwal | **Est:** 8 hrs | **Sub-task:** All controllers + Swagger UI + CORS configuration
...
**Owner:** Mehul Agarwal | **Est:** 12 hrs | **Sub-task:** MockMvc E2E tests + JaCoCo gate
...
**Problem:** Java's Bean convention strips the `is` prefix from boolean getters — Lombok/Jackson serialised `isFulfilled` as `fulfilled` in the JSON wire format. Early client models used `isFulfilled`, so statuses appeared wrong.
...
- Session state stored in `localStorage` and synchronized across browser tabs
...
### Challenge 7 — CORS Configuration for the React Client

**Problem:** The backend must allow the React development client without exposing the API to arbitrary origins.

**Solution:**
- Configured Spring's `CorsRegistry` to explicitly allowlist `http://localhost:5173`
- Used Vite's `server.proxy` to forward `/api/*` to `http://localhost:8080` in local development

---

### Challenge 8 — Dashboard Crashed for Users with No Owned Resources

**Problem:** The Dashboard rendered correctly for users who owned both a requirement and an offer, but became a blank screen for users with no owned resources or proposal-only activity. The browser network requests returned successfully, so the failure was in the React render path.

**Root cause:** `EmptyState` expects an icon component and renders it as `<IconComponent />`, while Dashboard passed already-rendered JSX such as `<ClipboardList />`. The empty-state branch therefore attempted to render an object as a component.

**Solution:**
- Passed icon components directly: `icon={ClipboardList}`, `icon={PackageOpen}`, and `icon={Handshake}`
- Changed Dashboard loading to use `Promise.allSettled` so one unavailable data section cannot hide the other sections
- Added explicit empty states for requirements, offers, and submitted proposals
- Added the target resource title to each submitted proposal for clearer tracking

**Lesson:** Empty and partial-data states must be tested as first-class workflows, not only the populated happy path. Component contracts should also distinguish between a component reference and an already-rendered element.
...
Using `useReducer` inside `AuthContext` (instead of plain `useState`) made state transitions explicit — `LOGIN`, `LOGOUT`, `UPDATE_EMPLOYEE` action types act as a mini-Redux. Combining with `localStorage` and the browser storage event keeps authentication state consistent across tabs without an additional state library.
...
**5. The API contract is the durable integration boundary**
The URL schema and JSON response structure are the most durable artefacts of the project. The React UI is a consumer of that contract.
